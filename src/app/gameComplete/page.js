...
function GameCompleteContent() {
  ...
  const formatTime = (seconds) => {
    ...
    if (mins > 0) {
      return `${mins} minut${mins !== 1 ? 'er' : ''} och ${secs} sekund${secs !== 1 ? 'er' : ''}`;
    }
        ...
        {success ? (
          <>
            ...
            <p className={styles.description}>
              Du klarade Level {level} med <strong>{formatTime(secondsLeft)}</strong> kvar 
              och var endast <strong>{distance} meter</strong> från målet! 
              Du fick därför <strong>{starsEarned} av 3 stjärnor!</strong>
            </p>
          </>
        ) : (
          <>
